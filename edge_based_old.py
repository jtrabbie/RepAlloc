...
# R = 2  # maximum number of repeaters
...
def solve_cplex(graph):
    ...
    print('Total number of variables: {} (expected {} variables)'
          .format(prob.variables.get_num(), (num_nodes * (num_nodes + 1) + 1) * len(unique_pairs) + num_nodes))
    prob.solve()
    print()
    chosen_paths = []
    repeater_nodes = []
...
def add_variables(prob, graph):
    ...
    for q in unique_pairs:
        ...
        for i in all_nodes:
            for j in all_nodes:
                if not (i == j or i == q[1] or j == q[0] or (type(i) != int and i not in q) or
                        (type(j) != int and j not in q)):
                    ...
                    if i == q[0]:  # Node i is the source
                        ...
                        if j == q[1]:  # Node j is the sink
                            ...
                            column = [cplex.SparsePair(ind=['FlowCon_' + pairname + "_" + str(i),
                                                            ...
                                                            'LinkCon_' + str(j)], val=[1.0, -1.0, 1.0])]
                    # Note that these variables have a lower bound of 0 by default
                    cplex_var = prob.variables.add(obj=[path_cost], columns=column,
